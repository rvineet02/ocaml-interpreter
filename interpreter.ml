...
and value =
  ...
  | VBool of bool
  | VName of string
  | VCLOSURE of env * string * string * cmds
...
let string_of_value v =
  ...
  | VName n -> n
  | VCLOSURE (env, fName, fArg, fCmds) -> "<fun>"
;;
...
let debug v =
  ...
  | VUnit -> "VUnit"
  | VInt i -> string_of_int i
  | VBool b -> if b then "V(True)" else "V(False)"
  | VName n -> n
;;
...
let switch_command integer bod log =
  let rec helper bod log =
    match bod with
    | CASE (i, commands) :: rest -> if integer = i then log, commands else helper rest log
    | _ -> [ "Error" ], []
  in
  helper bod log
;;

let fun_compute_local currentLocalEnv fName fArg fCmds =
  (fName, VCLOSURE (currentLocalEnv, fName, fArg, fCmds)) :: currentLocalEnv
;;

let call_compute_local currentLocalEnv fName fArg fCmds val_of_argument =
  (fArg, val_of_argument)
  :: (fName, VCLOSURE (currentLocalEnv, fName, fArg, fCmds))
  :: currentLocalEnv
;;
...
let rec eval (g : env) (l : env) (st : stack) (log : log) (cmds : cmds)
  ...
  | IfElse (cmds1, cmds2) :: cmds ->
    (match st with
    | VBool b :: st ->
      if b then eval g l st log (cmds1 @ cmds) else eval g l st log (cmds2 @ cmds)
    | _ -> g, log, None)
  | TRYEND bod :: cmds ->
    (match eval g l [] log bod with
    | g, log, Some (v :: _) -> eval g l (v :: st) log cmds
    | g, log, _ -> eval g l st log cmds)
  | SWITCHEND bod :: cmds ->
    (match st with
    | VInt integer :: newStack ->
      (match switch_command integer bod log with
      | newlog, newCommands ->
        if newlog = [ "Error" ]
        then g, log, None
        else eval g l newStack newlog (newCommands @ cmds))
    | _ -> g, log, None)
  | FUNCTION (funName, fArg, fCmds) :: rest ->
    (match funName, fArg with
    | Name fName, Name fArg ->
      let newLocalEnv = fun_compute_local l fName fArg fCmds in
      eval g newLocalEnv st log rest
    | _ -> g, log, None)
  | CALL :: rest ->
    (match st with
    | VCLOSURE (closureLocal, fName, fArg, fCmds) :: val_of_argument :: rem ->
      let newLocalEnv =
        call_compute_local closureLocal fName fArg fCmds val_of_argument
      in
      (match eval g newLocalEnv [] log fCmds with
      | g, log, Some (v :: _) -> eval g l (v :: rem) log rest
      | g, log, _ -> g, log, None)
    | _ -> g, log, None)
  | _ -> g, log, Some st
;;
...
let name : string parser =
  ...
  if List.exists (fun x -> x = s) reserved then fail else pure s << ws
;;
...
and try_parser () =
  ...
  let* commands = cmds_parser () in
  let* _ = keyword "End" in
  pure (TRYEND commands)

and cases_parser () =
  let* _ = keyword "Case" in
  let* n = natural << ws in
  let* commands = cmds_parser () in
  pure (CASE (n, commands))

and switch_parser () =
  let* _ = keyword "Switch" in
  let* cases = many (cases_parser ()) in
  let* _ = keyword "End" in
  pure (SWITCHEND cases)

and function_parser () =
  let* _ = keyword "Fun" in
  let* functionName = name_parser () in
  let* argName = name_parser () in
  let* functionCommands = cmds_parser () in
  let* _ = keyword "End" in
  pure (FUNCTION (functionName, argName, functionCommands))
